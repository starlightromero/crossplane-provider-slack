...
// Setup adds a controller that reconciles ConversationPin managed resources.
...
// SetupGated adds a controller that reconciles ConversationPin managed resources
// only if the EnableAlphaConversationPin feature flag is enabled.
func SetupGated(mgr ctrl.Manager, o controller.Options, f *feature.Flags) error {
	if f != nil && !f.Enabled(features.EnableAlphaConversationPin) {
		return nil
	}
	return setup(mgr, o)
}
...
func setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(pinv1alpha1.ConversationPinGroupKind)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(pinv1alpha1.ConversationPinGroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&pinv1alpha1.ConversationPin{}).
		Complete(r)
}
