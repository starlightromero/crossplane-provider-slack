...
// Setup adds a controller that reconciles UserGroupMembers managed resources.
...
// SetupGated adds a controller that reconciles UserGroupMembers managed resources
// only if the EnableAlphaUserGroupMembers feature flag is enabled.
func SetupGated(mgr ctrl.Manager, o controller.Options, f *feature.Flags) error {
	if f != nil && !f.Enabled(features.EnableAlphaUserGroupMembers) {
		return nil
	}
	return setup(mgr, o)
}
...
func setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(usergroupmembersv1alpha1.UserGroupMembersGroupKind)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(usergroupmembersv1alpha1.UserGroupMembersGroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&usergroupmembersv1alpha1.UserGroupMembers{}).
		Complete(r)
}
